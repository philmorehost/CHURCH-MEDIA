...
$stmt = $pdo->prepare("
    SELECT p.id, p.slug, p.caption, p.post_type, p.created_at, u.name AS author_name, u.username AS author_username,
...
$stmt->bindValue(count($unitIds) + 1, $perPage, PDO::PARAM_INT);
$stmt->execute();
$posts = $stmt->fetchAll();
